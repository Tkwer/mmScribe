...
import logging
from PySide6.QtCore import QMutex, QMutexLocker  # 使用 Qt 的互斥锁
...
dll = ctypes.cdll.LoadLibrary('libs/libuart_capture.so')
# 设置函数原型
...
class DataProcessor(QThread):
    ...
    def load_model(self):
        ...
        # Initialize NumPy and torch properly
        np.zeros(1)  # Force NumPy initialization
        import torch
        torch.manual_seed(0)  # Ensure reproducibility
        
        model_path = 'model/quantized_model/final.zip'  # Using the complete model package
        try:
            # Suppress warnings
            import warnings
