...
parula_data = [[0.2081, 0.1663, 0.5292], [0.2116238095, 0.1897809524, 0.5776761905], 
 ...
 [0.3481666667, 0.7424333333, 0.5472666667], [0.3952571429, 0.7459, 0.5244428571], 
 [0.4420095238, 0.7480809524, 0.5033142857], [0.4871238095, 0.7490619048, 0.4839761905], 
 ...
 [0.9697, 0.8481380952, 0.147452381], [0.9625857143, 0.8705142857, 0.1309], [0.9588714286, 0.8949, 0.1132428571], 
 [0.9598238095, 0.9218333333, 0.0948380952], [0.9661, 0.9514428571, 0.0755333333], [0.9763, 0.9831, 0.0538]]
# Create custom colormap

parula_map = LinearSegmentedColormap.from_list('parula', parula_data)

# Load matrix data
matrix = np.load("example/datas2/data_20240505_171357.npy")

# Slice the matrix into three parts
matrix_128 = matrix[:, :128]    # First 128 columns
matrix_32 = matrix[:, 128:160]  # Next 32 columns
matrix_3 = matrix[:, 160:]      # Last 3 columns (X-Y trajectory data)

# Create a figure with three subplots arranged in 1 row, 2 columns using GridSpec
fig = plt.figure(figsize=(18, 8))

# Define GridSpec layout
gs = gridspec.GridSpec(3, 2, width_ratios=[1, 2])  
duration = len(matrix_128[:, 0]) * 0.0096  # Calculate duration for time axis

# Create time and frequency axes
time_axis = np.linspace(0, duration, matrix_3.shape[0] + 1)  
freq_128 = np.linspace(20, matrix_128.shape[1] + 20, matrix_128.shape[1] + 1)
freq_32 = np.linspace(20, matrix_32.shape[1] + 20, matrix_32.shape[1] + 1)

# First subplot: X-Y trajectory (spans all rows of the first column)
ax_trajectory = fig.add_subplot(gs[0:3, 0])  
ax_trajectory.set_title('X-Y Trajectory', fontsize=20)  
ax_trajectory.plot(matrix_3[:, 0], matrix_3[:, 1], color='black', linewidth=8)
ax_trajectory.set_ylim(-50, 550) 
ax_trajectory.set_xticks([])  # Disable x-axis ticks
ax_trajectory.set_yticks([])  # Disable y-axis ticks
for spine in ax_trajectory.spines.values():  
    spine.set_linewidth(2)  

# Second subplot: Doppler-time map
ax_doppler = fig.add_subplot(gs[0, 1])
doppler_mesh = ax_doppler.pcolormesh(time_axis, freq_128, matrix_128.T, cmap=parula_map, vmin=0, vmax=1.2)
ax_doppler.set_ylabel('Frequency (Hz)', fontsize=16)
ax_doppler.set_xticks([])  
for spine in ax_doppler.spines.values():  
    spine.set_linewidth(2)

# Third subplot: Magnitude-time map
ax_magnitude = fig.add_subplot(gs[1, 1])
magnitude_mesh = ax_magnitude.pcolormesh(time_axis, freq_32, matrix_32.T, cmap=parula_map, vmin=0, vmax=1.2)
ax_magnitude.set_ylabel('Frequency (Hz)', fontsize=16)
ax_magnitude.set_xticks([])  
for spine in ax_magnitude.spines.values():  
    spine.set_linewidth(2)

# Colorbars
cbar_ax = fig.add_axes([0.92, 0.6, 0.02, 0.3])
fig.colorbar(doppler_mesh, cax=cbar_ax)

# Adjust the layout to fit the subplots neatly
